...
from util import Context, guard_logged_in, guard_session, depends_session, depends_user, ApiException
from models import GrockyList, TaskListItem, GroceryListItem, ListItem, OwnerDescriptor, User, AssembledList
...
class ListsController(Controller):
    ...
    @post("/")
    async def create_list(self, data: ListCreationModel, context: Context) -> GrockyList:
        new_list = GrockyList(
            id=GrockyList._id(),
    ...
    async def get_user_lists(self, context: Context, user: User) -> list[GrockyList]:
        lists: list[GrockyList] = GrockyList.load_query(context.database, {"owned_by": {"type": "user", "id": user.id}})
    ...
    async def get_list(self, list_item: GrockyList) -> list[Union[TaskListItem, GroceryListItem, ListItem]]:
