...
from util import guard_logged_in, guard_session, depends_user, depends_session, Context, ApiException
...
class UserController(Controller):
    ...
    @post("/settings")
    async def update_user_settings(self, user: User, data: dict, context: Context) -> RedactedUser:
        ...
        if resolved_updates.username != user.username:
            existence_check = User.load_query(context.database, {"username": resolved_updates.username})
        ...
        if resolved_updates.display_name != user.display_name and len(resolved_updates.display_name) == 0:
    ...
    @post("/password")
    async def change_user_password(self, user: User, data: PasswordChangeModel) -> RedactedUser:
